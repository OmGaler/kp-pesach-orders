...
function buildSalesAnalysis(orders, metadataLookup = new Map()) {
  ...
  return {
    ...
    unfulfilledProductTotals: buildUnfulfilledProductTotals(orders, metadataLookup),
...
function buildUnfulfilledProductTotals(orders, metadataLookup = new Map()) {
  const productTotals = new Map();
  ...
  const totalQuantity = Array.from(productTotals.values()).reduce(
